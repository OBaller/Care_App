...
extension UITapGestureRecognizer {
    ...
    func didTapAttributedTextInLabel(label: UILabel, inRange targetRange: NSRange) -> Bool {
      ...
      let textStorage = NSTextStorage(attributedString: label.attributedText ?? NSAttributedString())
        ...
        let textContainerOffset = CGPoint(x: (labelSize.width - textBoundingBox.size.width)
                                          * 0.5 - textBoundingBox.origin.x, y: (labelSize.height - textBoundingBox.size.height)
